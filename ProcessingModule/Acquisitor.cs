    ...
    public class Acquisitor : IDisposable
	{
		...
		private void Acquisition_DoWork()
		{
            ...
            while (true)
            {
                ...
                foreach(IConfigItem item in configItemList)
                {
                    item.SecondsPassedSinceLastPoll++;
                    int acq = item.AcquisitionInterval;

                    if(item.SecondsPassedSinceLastPoll == acq)
                    {
                        processingManager.ExecuteReadCommand(
                            item, this.configuration.GetTransactionId(), this.configuration.UnitAddress, item.StartAddress,
